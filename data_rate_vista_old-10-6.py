...
def occupancy_volume(input: tuple) -> np.float32:
    ...
    Args:
        input (tuple): Tuple of form (pc, r_size, azimuth_size, elevation_size, sensorcon, i).
    ...
    if writeVolume:
        # Debug to write volume to csv
        sample_out = np.vstack((volume, vx[:,2], vx[:,1], vx[:,0])).T
        df = pd.DataFrame(volume)
        #df.columns = ['volume', 'azimuth', 'elevation', 'range']
        df.columns = ['volume']
        output_file = f"volume_{i}.txt"
...
def main():
    ...
    if writeVolume:
        # Global source name for writing voxel volumes to csv
        global output_folder
        filename = os.path.basename(os.path.normpath(path2scenes))
        output_folder = f"./examples/voxelized_volumes/{filename}"
